...
The architectural side-benefit: the PHP-stack ceiling no longer caps JS recursion depth. `CallStack::maxDepth` is the only limit now (65,536, well above V8 / SpiderMonkey practical stack-overflow thresholds).
...
Closing the remaining gap with V8 would require a proper PHP-bytecode JIT — generating opcache-friendly PHP that the host's tracing JIT (PHP 8.5+) can compile to native. That's on the long-term roadmap.
...
`bench/microbench.js` runs a dozen tiny benchmarks (loop-arith, loop-fib, fn-recurse, fn-deep-recurse, obj-create, obj-prop, proto-method, arr-push, arr-map, str-concat, str-split-join, json-roundtrip, closure, destructure). Median wall time:
