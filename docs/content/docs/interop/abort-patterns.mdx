...
`AbortSignal.timeout(ms)` returns a signal that aborts itself after `ms`. Note Phasis has no host event loop yet, so the timeout is **lazy** — it triggers the next time something reads `signal.aborted` or the fetch transport polls. For fetch transports that poll the signal during transfer (the default `CurlTransport` does, via `CURLOPT_PROGRESSFUNCTION`), this means the request *will* be aborted close to the deadline.

For pure-JS code that just sleeps in a Promise loop with no polling, the timeout will never fire — there's no scheduler to wake.
...
- **`setTimeout(() => ctrl.abort(), 100)`** — `setTimeout` isn't shipped yet. Use `AbortSignal.timeout(100)` instead.
