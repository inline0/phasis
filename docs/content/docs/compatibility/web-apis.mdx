...
WPT fixtures imported into `tests/Wpt/fixtures/` cover the Web Platform Pack and Fetch Pack:

| Fixture area | Fixtures |
|---|---:|
| `abort/` (AbortController / AbortSignal) | 4 |
| `atob/` | 1 |
| `blob/` (Blob, File slicing, byte semantics) | 8 |
| `encoding/` (TextEncoder / TextDecoder) | 2 |
| `events/` (EventTarget, Event, propagation) | 8 |
| `file/` | 1 |
| `formdata/` | 9 |
| `headers/` | 12 |
| `hr-time/` (`performance.now`) | 1 |
| `streams/` (Readable / Writable / Transform + BYOB + queuing) | 7 |
| `structured-clone/` | 1 |
| `url/` (URL, URLSearchParams) | 2 |
| **Total** | **56 fixtures · 815 subtests · 100 %** |

`crypto` / `SubtleCrypto`, `WebSocket`, and `XMLHttpRequest` ship in the engine but don't yet have WPT fixtures imported. They're verified against Node.js via [popular-package tests](/docs/compatibility/popular-packages) and the unit-test suite.

Verified by `bin/wpt` against the official [Web Platform Tests](https://github.com/web-platform-tests/wpt). Run yourself:
...
The local WPT clone at `tests/Wpt/upstream/` is a sparse-checkout of [web-platform-tests/wpt](https://github.com/web-platform-tests/wpt) covering `fetch/`, `streams/`, `FileAPI/`, `xhr/formdata/`, `dom/abort/`, `dom/events/`. 56 fixtures are currently imported into `tests/Wpt/fixtures/` across 12 areas.

To import more: copy any `*.any.js` file from `upstream/<area>/` into `tests/Wpt/fixtures/<category>/`. The runner picks it up on next run. Failures get logged via `bin/wpt --verbose`.
