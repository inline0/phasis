...
Beyond ECMAScript itself, Phasis ships two layered packs of WHATWG / W3C APIs:
...
Both packs are **on by default**. No flag, no setup — `typeof fetch` returns `"function"` on a fresh `new Engine()`.

## Conformance

| Area | Subtests | Status |
|---|---:|---|
| `URL` / `URLSearchParams` | 20 | 100 % |
| `TextEncoder` / `TextDecoder` | 15 | 100 % |
| `atob` / `btoa` | 6 | 100 % |
| `structuredClone` | 11 | 100 % |
| `performance.now` | 3 | 100 % |
| `Headers` | 132 | 100 % |
| `Blob` / `File` | 235 | 100 % |
| `AbortController` / `AbortSignal` / `Event` | 22 | 100 % |
| **Total** | **444** | **100 %** |
...
setTimeout(() => ctrl.abort(), 5000);  // (setTimeout not yet shipped; abort manually instead)
try {
...
The local WPT clone at `tests/Wpt/upstream/` is a sparse-checkout of [web-platform-tests/wpt](https://github.com/web-platform-tests/wpt) covering `fetch/`, `streams/`, `FileAPI/`, `xhr/formdata/`, `dom/abort/`, `dom/events/`. 304 fixtures available; 23 currently imported into `tests/Wpt/fixtures/`.
