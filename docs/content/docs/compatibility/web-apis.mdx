...
`crypto` / `SubtleCrypto`, `WebSocket`, and `XMLHttpRequest` ship in the engine but don't yet have WPT fixtures imported — they're verified against Node.js via the [popular-package tests](/docs/compatibility/popular-packages) and the PHPUnit suite. Run the WPT corpus yourself:
