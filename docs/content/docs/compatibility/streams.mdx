...
- **No timer-based scheduling.** Phasis has no host event loop yet (no `setTimeout`/`setInterval`), so streams that depend on time-spaced production must drive their own scheduling. `Promise.resolve().then(...)` microtask draining works.
- **Synchronous-only producers.** A `pull()` that performs blocking PHP I/O will block the script. Use the fetch transport hook for HTTP; for files, prefer reading into memory upfront.
