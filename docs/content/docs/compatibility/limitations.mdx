...
Phasis ships the full **Web Platform Pack** (`URL`, `TextEncoder`/`TextDecoder`, `atob`/`btoa`, `structuredClone`, `performance`, `DOMException`) and the full **Fetch Pack** (`fetch`, `Request`, `Response`, `Headers`, `Body`, `AbortController`/`AbortSignal`, `Blob`/`File`, `FormData`, `EventTarget`/`Event`, full WHATWG Streams, `navigator`) — see [Web APIs](/docs/compatibility/web-apis).
...
- `WebSocket` — separate transport. Possible to add; not yet on the roadmap.
- `Worker`, `SharedWorker`, `MessageChannel` — needs real OS threads, which PHP doesn't offer.
- `setTimeout`, `setInterval`, `queueMicrotask` — needs an event loop. Phasis is synchronous-embeddable today; a runtime-model change would be required.
- `crypto.subtle`, `crypto.getRandomValues` — separate threat model; possible add later.
- `XMLHttpRequest` — superseded by `fetch`, which we *do* ship.
