...
The engine is a tree-walker with an opportunistic bytecode VM. Most hot dispatch happens inside the VM's switch loop — not by walking the AST — but each VM opcode still pays a PHP dispatch tax. Even with the custom callstack (one PHP frame per execute() invocation rather than per JS call), the inline cache for prototype reads, and frame-snapshot generators, Phasis sits roughly **~100× slower than V8** on dispatch-bound workloads.
...
A future bytecode JIT (lowering hot JS functions to opcache-friendly PHP that the host's tracing JIT can compile to native) could close more of this gap. It's on the roadmap, not yet shipped.
