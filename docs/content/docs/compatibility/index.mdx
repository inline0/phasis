...
3. **Popular libraries** — `acorn`, `mustache.js`, `lodash`, `marked`, `dayjs`, `bignumber.js`, `json5`, `fuse.js` execute byte-equal to Node.js on every push.
...
- **[Popular packages](/docs/compatibility/popular-packages)** — CI-gated byte-equality with Node.js for acorn, mustache.js, lodash, marked, dayjs, bignumber.js, json5, fuse.js.
