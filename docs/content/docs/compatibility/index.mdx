...
3. **Popular libraries** — 66 widely-used npm packages (acorn, lodash, zod, ramda, immer, redux, date-fns, marked, fuse.js, semver, …) execute byte-equal to Node.js on every push.
...
- **[Popular packages](/docs/compatibility/popular-packages)** — CI-gated byte-equality with Node.js for 66 npm packages across parsing, templating, validation, date/time, math, hashing, search/diff, color, URL handling, and more.
