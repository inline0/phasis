...
2. **Web Platform Tests** — the official WHATWG / W3C suite for non-ECMAScript Web APIs (fetch, Streams, Headers, Blob, encoding, URL, structured-clone, abort, hr-time, atob). **100 %** on the imported corpus.
3. **Popular libraries** — `acorn`, `mustache.js`, `lodash`, `marked` execute byte-equal to Node.js on every push.
