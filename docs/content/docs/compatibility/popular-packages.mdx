...
description: Phasis runs 100+ real-world JS libraries — acorn, lodash, zod, marked, ramda, dayjs, date-fns, immer, redux, immutable, fuse.js, minisearch, semver, and more — byte-equal to Node.js. Verified on every push via the popular.yml CI workflow.
---
...
Phasis verifies **104 widely-used libraries** byte-for-byte against Node.js (V8) on every push. They live under `tests/Popular/` in the repo.
...
Total: **104 libraries** all reproduce Node's exact output. All pinned to the latest upstream release as of the last bump.
