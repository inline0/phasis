...
description: Phasis runs real-world JS libraries — acorn, mustache.js, lodash, marked, dayjs, bignumber.js, json5, fuse.js, handlebars, zod — byte-equal to Node.js. Verified on every push via the popular.yml CI workflow.
---
...
Phasis verifies ten widely-used libraries byte-for-byte against Node.js (V8) on every push. They live under `tests/Popular/` in the repo.

## Coverage

| Library | Version | Surface exercised | CI gate |
|---|---|---|---|
| **[acorn](https://github.com/acornjs/acorn)** | 8.16.0 | Modern-ES parser: classes with private fields, arrow + async generator + yield-await, destructuring with default/rest, tagged template (`String.raw`), import/export, ES2024 syntax. 126-line AST summary emitted via deterministic walker. | byte-equal |
...
| **lodash** | 4.18.1 | Full utility surface: array methods (`map`, `filter`, `reduce`, `flat`, `flatten`, `chunk`, `uniq`, `zip`, `union`, `intersection`), collection (`sortBy`, `orderBy`, `groupBy`, `keyBy`), object (`pick`, `omit`, `get`, `set`, `merge`, `defaults`, `invert`), lang (`isEqual`, `cloneDeep`, `isMatch`), string transforms, math reducers, function helpers (`memoize`, `once`), `_.chain`. ~60 invocations. | byte-equal |
| **marked** | 18.0.3 | Markdown parser. Headings, inline emphasis, lists (ordered + unordered + nested), code blocks (fenced + indented), block-quotes, tables, HTML passthrough, combined readme. | byte-equal |
| **[dayjs](https://day.js.org)** | 1.11.20 | Date arithmetic + formatting. Constructors from ISO / epoch ms / `Date`, `add` / `subtract` across day / month / year, `diff` in absolute units, `startOf` / `endOf`, format-token output (`YYYY-MM-DD`, `HH:mm:ss`, etc.), comparison operators, immutability. Pinned to `date.timezone=UTC` so the oracle is byte-stable across runners. | byte-equal |
| **[bignumber.js](https://mikemcl.github.io/bignumber.js/)** | 11.1.1 | Arbitrary-precision arithmetic. `plus` / `minus` / `times` / `div` / `mod` on 30-digit operands, `pow` / `sqrt`, rounding modes (`HALF_UP` / `HALF_DOWN` / `FLOOR` / `CEIL`), `decimalPlaces`, base conversion (`toString(2)` / `toString(16)`), scientific notation, factorial(15). | byte-equal |
...
| **[fuse.js](https://fusejs.io)** | 7.3.0 | Fuzzy search. Exact match, typo-tolerant match (Bitap), nested-key lookup (`author.last`), per-key weighting, tags-as-array indexing, threshold tuning (0.0 exact → 0.3 near-exact). Babel-transpiled bundle that exercises `_classCallCheck` (`this instanceof Ctor` after factory-IIFE return) and `var` hoisting inside `else if` blocks. Vendored as an esbuild IIFE (upstream dropped UMD at 7.1). | byte-equal |
| **[handlebars](https://handlebarsjs.com)** | 4.7.9 | Template compile + render. Plain interpolation, triple-stache escape skip, `{{#if}}` / `{{else}}` blocks, `{{#each}}` over arrays and objects (with `@index` / `@key` / `@first` / `@last`), `{{#with}}`, built-in `lookup` helper, custom helper + `SafeString`, block helpers, partials (`{{> name}}`), parent-context paths (`{{../prefix}}`), subexpressions, compile-once / render-many reuse. | byte-equal |
| **[zod](https://zod.dev)** | 4.4.3 | Runtime type validation. Primitive schemas with refinements (`.min`/`.max`/`.email`/`.int`/`.positive`), object schemas with optional / nullable fields, array length constraints, discriminated unions, `.transform()` pipelines, `.default()` values, custom `.refine()` predicates, `z.enum` membership, structured `ZodError.issues` (path + code per failure). Vendored as an esbuild IIFE (upstream dropped UMD at 3.25). | byte-equal |

Total: **10 libraries · ~1.2 MB of vendored upstream JS** all reproduce Node's exact output. All pinned to the latest upstream release as of the last bump.
