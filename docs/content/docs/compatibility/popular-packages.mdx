...
description: Phasis runs real-world JS libraries — acorn, mustache.js, lodash, marked — byte-equal to Node.js. Verified on every push via the popular.yml CI workflow.
---
...
Phasis verifies four widely-used libraries byte-for-byte against Node.js (V8) on every push. They live under `tests/Popular/` in the repo.
...
Total: **4 libraries · 384 KB of vendored upstream JS · ~250 individual library calls** all reproduce Node's exact output.
...
Strong candidates worth adding: `chevrotain` (parser combinator, exercises closures + regex), `nanoid` (crypto/randomness boundary), `date-fns` (Date/timezone surface), `zod` (schema/error formatting), `handlebars` (template compile + cache).
