...
- **Spec-compliant** — 100 % of the official test262 suite passes (50,506 / 50,506).
- **Web APIs** — full **Web Platform Pack** (URL, TextEncoder/Decoder, atob/btoa, structuredClone, performance, DOMException) and **Fetch Pack** (fetch, Request, Response, Headers, Body, AbortController/Signal, Blob, File, FormData, EventTarget, full WHATWG Streams, navigator). 100 % of imported Web Platform Tests pass.
...
- Verifies every change against Node.js (V8). Compliance is measured continuously against the official test262 suite in a 73-shard CI matrix, plus the Web Platform Tests via `bin/wpt`.
